...
def comentariosactividad(n1):  
    ...
    id_actividad = n1
    db = get_db()
    mensajesfull =  db.execute('SELECT mensaje,nombre_usuario,Apellido_usuario FROM rel_mensajes_actividades_usuario INNER JOIN usuario ON usuario.id_usuario = rel_mensajes_actividades_usuario.id_usuario WHERE rel_mensajes_actividades_usuario.id_actividad = ? AND eliminar = 0 ',(id_actividad,)).fetchall()
    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if error is not None:
                ...
                db = get_db()
                db.execute('INSERT INTO rel_mensajes_actividades_usuario (id_actividad, id_usuario, mensaje, eliminar) VALUES (?, ?, ?, ?)',(n1, logueadouser, mensaje, 0))
                db.commit()
...
def eliminarcomentario(idActividad,idComentario):
    ...
    if request.method == 'GET': 
        if error is not None:
            ...
            db = get_db()
            db.execute('UPDATE rel_mensajes_actividades_usuario SET eliminar = 1 WHERE id = ?',(idComentario,))
            db.commit()
...
def editarcomentario(idActividad,idComentario):
    ...
    if request.method == 'GET': 
        if error is not None:
            ...
            db = get_db()
            consulta = db.execute('SELECT mensaje FROM rel_mensajes_actividades_usuario WHERE id = ? AND eliminar = 0',(idComentario,)).fetchone()
